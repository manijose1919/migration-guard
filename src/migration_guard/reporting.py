...
def render_rules_markdown(catalog: list[dict] | None = None) -> str:
    """Render the rule catalog as a Markdown reference page (RULES.md)."""
    cat = catalog if catalog is not None else rule_catalog()
    lines = [
        "# MigrationGuard rules",
        "",
        _RULES_DOC_HEADER,
        "",
        f"{len(cat)} rules. Severity feeds the `--fail-on` gate; **Dialects** shows "
        "where a rule applies; **Fix** marks rules that `--fix` can rewrite automatically.",
        "",
    ...
    for r in cat:
        ...
        fix = "auto" if r["fixable"] else "—"
        lines.append(
            f"| {r['id']} | {r['default_severity']} | `{r['name']}` | "
            f"{dialects} | {fix} | {r['summary']} |"
        )
    return "\n".join(lines) + "\n"
